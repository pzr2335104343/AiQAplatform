...
import com.pzr.rongdada.annotation.AuthCheck;
import com.pzr.rongdada.common.BaseResponse;
import com.pzr.rongdada.common.DeleteRequest;
import com.pzr.rongdada.common.ErrorCode;
import com.pzr.rongdada.common.ResultUtils;
import com.pzr.rongdada.constant.UserConstant;
...
import com.pzr.rongdada.model.dto.question.*;
...
import com.pzr.rongdada.model.enums.AppTypeEnum;
import com.pzr.rongdada.model.vo.QuestionVO;
import com.pzr.rongdada.service.AppService;
...
public class QuestionController {
    ...
    private static final String GENERATE_RESULT_QUESTION_SYSTEM_MESSAGE = "你是一位严谨的出题专家,我会给你如下信息：\n" +
            "```\n" +
            "应用名称,\n" +
            "【【【应用描述】】】,\n" +
            "应用类别,\n" +
            ...
            "请你根据上述信息,按照以下步骤来出题：\n" +
            "1. 要求：题目和选项尽可能地短,题目不要包含序号,每题的选项数以我提供的为主,题目不能重复\n" +
            "2. 根据 应用类别 严格按照下面两个的 json 格式输出题目和选项\n" +
            " (1)当应用类别是测评类,使用以下结构\n" +
            "    ```\n" +
            "    [{options:[{value:选项内容,key:A,result:人格属性},{value:,key:B,result:人格属性}],title:题目标题}]\n" +
            "    ```\n" +
            "    title 是题目,options 是选项,每个选项的 key 按照英文字母序（比如 A、B、C、D）以此类推,value 是选项内容,result是人格属性（比如：外向为E,内向为I 等等）\n" +
            " (2)当应用类别是得分类,使用以下结构\n" +
            "    ```\n" +
            "    [{options:[{value:选项内容,key:A,score:选项得分},{value:,key:B,score:选项得分}],title:题目标题}]\n" +
            "    ```\n" +
            "    title 是题目,options 是选项,每个选项的 key 按照英文字母序（比如 A、B、C、D）以此类推,value 是选项内容,score是选项得分(得分范围0-10)\n" +
            "3. 检查题目是否包含序号,若包含序号则去除序号\n" +
            "4. 返回的题目列表格式必须为 JSON 数组\n" +
            "5. 题目要尽可能的不类似\n" +
            "6. 检查题目数量是否正确;";
    ...
    public BaseResponse<List<QuestionContentDTO>> aiGenerateQuestion(
        ...
        // AI 生成
        String result = aiManager.doSyncStableRequest(GENERATE_RESULT_QUESTION_SYSTEM_MESSAGE, userMessage);
        // 截取需要的 JSON 信息
    ...
    public SseEmitter aiSseGenerateQuestion(AiGenerateQuestionRequest aiGenerateQuestionRequest) {
        ...
        // AI 生成,SSE流式返回
        Flowable<ModelData> modelDataFlowable = aiManager.doStreamRequest(GENERATE_RESULT_QUESTION_SYSTEM_MESSAGE, userMessage, 0.95f);
    ...
    public SseEmitter aiSseGenerateQuestionTest(AiGenerateQuestionRequest aiGenerateQuestionRequest, boolean isVip) {
        ThrowUtils.throwIf(aiGenerateQuestionRequest == null, ErrorCode.PARAMS_ERROR);
        // 获取参数
        Long appId = aiGenerateQuestionRequest.getAppId();
        int questionNumber = aiGenerateQuestionRequest.getQuestionNumber();
